  ...
  <div class="asset-page">
    ...
    <div class="asset-page-header">
      <div>
        ...
        <p class="asset-page-desc">Pantau dan kelola aset organisasi berdasarkan unit kerja.</p>
      </div>
    </div>
    ...
    <div v-if="!selectedUnitKerjaId && !ukLoading" class="asset-empty-prompt">
      ...
      <p v-else class="asset-empty-text">Pilih unit kerja untuk melihat daftar aset</p>
    </div>
      ...
      <div class="asset-controls">
        <div class="asset-filters">
          ...
          <div class="filter-select-wrap">
            <select v-model="filterCategoryId" class="filter-select" @change="onFilterChange">
        ...
        <Button label="Tambah Aset" icon="pi pi-plus" size="small" @click="openCreate" />
      </div>
      ...
      <div class="asset-table-wrap">
        ...
        <template v-else>
            ...
            <tbody>
              ...
              <tr v-for="asset in assets" :key="asset.id">
                <td class="asset-td-name">
                  {{ asset.name }}
                  <div v-if="asset.description" class="asset-td-desc">{{ asset.description }}</div>
                </td>
                <td><span class="asset-code-chip">{{ asset.code }}</span></td>
                <td class="asset-td-dim">{{ asset.category.name }}</td>
                <td class="asset-td-dim">{{ asset.owner || '—' }}</td>
                <td class="asset-th-status">
                  <span class="asset-status-chip" :class="`status-${asset.status.toLowerCase()}`">
          ...
          <div v-if="pagination" class="asset-total-count">
            Menampilkan {{ assets.length }} dari {{ pagination.totalItems }} aset
          </div>
        </template>
      </div>
    </template>
    ...
    >
      <form class="asset-form" @submit.prevent="submitForm">
        ...
        <div class="form-group">
          ...
          <div class="asset-select-wrap" :class="{ 'is-error': formErrors.categoryId }">
          ...
          <span v-if="formErrors.categoryId" class="form-err">{{ formErrors.categoryId }}</span>
        </div>

        <!-- Owner -->
        <div class="form-group">
          <label class="form-label">Pemilik / Penanggungjawab <span class="form-opt">(opsional)</span></label>
          <input
            v-model="form.owner"
            class="asset-input"
            type="text"
    ...
    <Dialog
      ...
      modal
      :header="statusAction === 'activate' ? 'Aktifkan Aset' : 'Nonaktifkan Aset'"
      :style="{ width: '400px' }"
    >
    ...
    >
      <div class="confirm-body">
        ...
        <p class="confirm-warn">
          Aset yang diarsipkan tidak dapat diaktifkan kembali. Pastikan aset sudah benar-benar
          tidak digunakan sebelum diarsipkan.
        </p>
...
const form = reactive({ name: '', code: '', categoryId: '', owner: '', description: '', status: 'ACTIVE' as AssetStatus })
...
async function submitForm() {
  ...
  try {
    ...
    if (editTarget.value) {
      await assetApi.update(selectedUnitKerjaId.value, editTarget.value.id, payload)
      toast.add({ severity: 'success', summary: 'Berhasil', detail: 'Aset berhasil diperbarui', life: 3000 })
    } else {
      await assetApi.create(selectedUnitKerjaId.value, { ...payload, status: form.status })
      toast.add({ severity: 'success', summary: 'Berhasil', detail: 'Aset berhasil ditambahkan', life: 3000 })
    }
...
async function submitStatusChange() {
  ...
  try {
    if (statusAction.value === 'activate') {
      ...
      toast.add({ severity: 'success', summary: 'Berhasil', detail: 'Aset berhasil diaktifkan', life: 3000 })
    } else {
      await assetApi.deactivate(selectedUnitKerjaId.value, statusTarget.value.id)
      toast.add({ severity: 'success', summary: 'Berhasil', detail: 'Aset berhasil dinonaktifkan', life: 3000 })
    }
...
async function submitArchive() {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Berhasil', detail: 'Aset berhasil diarsipkan', life: 3000 })
