  ...
  <div class="rpd-page">
    ...
    <template v-else-if="program">
      ...
      <div class="rpd-breadcrumb-row">
        <span
          class="rpd-breadcrumb-text"
          @click="router.push({ name: 'risk-programs' })"
          >Program Risiko</span
        >
          ...
          <div class="rpd-sidebar-card">
            <div class="rpd-sidebar-header">
              ...
              <div class="rpd-sidebar-badges">
                ...
                <span
                  class="rpd-status-chip"
                  :class="`chip-${program.status.toLowerCase()}`"
                >
                  {{ RISK_PROGRAM_STATUS_LABELS[program.status] }}
                </span>
              </div>
              <p v-if="program.description" class="rpd-sidebar-desc">
                {{ program.description }}
              </p>
              <p v-else class="rpd-sidebar-desc rpd-no-desc">
                Belum ada deskripsi.
              </p>
            </div>
        ...
        <main class="rpd-main">
          ...
          <section class="rpd-section">
            <div class="rpd-section-header">
              <div class="rpd-section-header-left">
                ...
                <p class="rpd-section-desc">
                  Pilih framework standar yang digunakan sebagai acuan analisis
                  risiko pada program ini.
                </p>
              </div>
              <button
                class="rpd-add-btn"
                type="button"
                @click="openAddFramework"
              >
            ...
            <div v-else-if="programFrameworks.length === 0" class="rpd-empty">
              ...
              <p class="rpd-empty-desc">
                Tambahkan framework untuk mulai menyusun konteks risiko dalam
                program ini.
              </p>
              <button
                class="rpd-add-btn"
              ...
              >
                ...
                <div class="rpd-fw-row-left">
                  <div class="rpd-fw-row-top">
                    ...
                    <span v-if="pf.framework.version" class="rpd-fw-version"
                      >v{{ pf.framework.version }}</span
                    >
                    <span
                      class="rpd-fw-status"
                      :class="`fw-status-${pf.status.toLowerCase()}`"
                    >
                  ...
                  <div class="rpd-fw-row-meta">
                    <span class="rpd-fw-ctx-count">
                      <i class="pi pi-file" />
                      {{ pf._count?.programFrameworkContexts ?? 0 }} konteks
                      dipilih
                    </span>
                  </div>
                </div>
                  ...
                  <button
                    ...
                    @click="
                      router.push({
                        ...
                        params: { id: programId, frameworkId: pf.id },
                      })
                    "
                  >
    ...
    <!-- ─── Edit Program Dialog ───────────────────────────────────────────────── -->
    <Dialog
      v-model:visible="showEditProgram"
      modal
      header="Edit Program Risiko"
      :style="{ width: '480px' }"
    >
      <form class="rpd-form" @submit.prevent="submitEditProgram">
        <div class="form-group">
          ...
          <input
            ...
            autocomplete="off"
          />
          <span v-if="programFormErrors.name" class="form-err">{{
        ...
        <div class="form-row-2">
          <div class="form-group">
            ...
            <input
              ...
              autocomplete="off"
            />
            <span v-if="programFormErrors.year" class="form-err">{{
    ...
    <Dialog
      v-model:visible="showAddFw"
      modal
      header="Tambah Framework ke Program"
      :style="{ width: '480px' }"
    >
      <form class="rpd-form" @submit.prevent="submitAddFramework">
        <div class="form-group">
          ...
          <span v-if="addFwErrors.frameworkId" class="form-err">{{
            addFwErrors.frameworkId
          }}</span>
          <span
            v-if="availableFrameworks.length === 0 && !fwListLoading"
            class="form-hint"
          >
        ...
        <div class="rpd-form-actions">
          <Button
            ...
            severity="secondary"
            type="button"
            :disabled="addFwLoading"
            @click="showAddFw = false"
          />
    ...
    <Dialog
      v-model:visible="showEditFw"
      modal
      header="Edit Status Framework"
      :style="{ width: '420px' }"
    >
      <form class="rpd-form" @submit.prevent="submitEditFramework">
        ...
        <div class="form-group">
          <label class="form-label">Status</label>
          <select
            v-model="editFwForm.status"
            class="rpd-field-input rpd-field-select"
          >
            <option
              v-for="(label, val) in PROGRAM_FRAMEWORK_STATUS_LABELS"
              :key="val"
              :value="val"
            >
              {{ label }}
        ...
        <div class="rpd-form-actions">
          <Button
            ...
            severity="secondary"
            type="button"
            :disabled="editFwLoading"
            @click="showEditFw = false"
          />
    ...
    <Dialog
      v-model:visible="showDeleteFw"
      modal
      header="Hapus Framework dari Program"
      :style="{ width: '400px' }"
    >
      <div class="del-body">
        ...
        <p class="del-text">
          Hapus <strong>{{ deleteFwTarget?.framework.name }}</strong> dari
          program ini?
        </p>
        <p class="del-warn">
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          :disabled="deleteFwLoading"
          @click="showDeleteFw = false"
        />
        <Button
          label="Hapus"
          severity="danger"
          :loading="deleteFwLoading"
          @click="submitDeleteFramework"
        />
      </template>
    </Dialog>
  </div>
...
async function loadProgram() {
  ...
  try {
    ...
    programError.value = extractApiError(
      err,
...
async function submitEditProgram() {
  ...
  if (!programForm.name.trim()) {
    programFormErrors.name = "Nama wajib diisi";
    valid = false;
  }
  if (!programForm.year || programForm.year < 2000 || programForm.year > 2100) {
    programFormErrors.year = "Tahun tidak valid";
    valid = false;
  }
  if (!valid) return;
...
async function submitAddFramework() {
  addFwErrors.frameworkId = "";
  if (!addFwForm.frameworkId) {
    addFwErrors.frameworkId = "Framework wajib dipilih";
    return;
  }
  ...
  addFwError.value = "";
  try {
    await programFrameworkApi.add(programId, {
      frameworkId: addFwForm.frameworkId,
    });
    showAddFw.value = false;
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Framework berhasil ditambahkan",
      life: 3000,
    });
    loadFrameworks();
    loadProgram();
...
async function submitEditFramework() {
  ...
  editFwError.value = "";
  try {
    await programFrameworkApi.update(programId, editFwTarget.value.id, {
      status: editFwForm.status,
    });
    showEditFw.value = false;
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Konfigurasi framework diperbarui",
      life: 3000,
    });
    loadFrameworks();
  } catch (err: any) {
...
async function submitDeleteFramework() {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Framework dihapus dari program",
      life: 3000,
    });
    loadFrameworks();
    loadProgram();
