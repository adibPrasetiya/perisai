    ...
    <template v-else>
        ...
        <div class="rcc-header-left">
          ...
          <div>
            ...
            <div class="rcc-subtitle">
              ...
              <span v-if="fw?.version" class="rcc-fw-version"
                >v{{ fw.version }}</span
              >
            </div>
          </div>
      ...
      <div class="rcc-steps">
        <div v-for="(title, i) in STEP_TITLES" :key="i" class="rcc-step-wrap">
          ...
          <div
            v-if="i < STEP_TITLES.length - 1"
            class="rcc-step-line"
            :class="{ 'line-done': step > i + 1 }"
          />
        </div>
      </div>
            ...
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Identitas utama konteks risiko dalam framework ini.
                </div>
              </div>
            </div>
              ...
              <div class="form-row-2">
                <div class="form-group">
                  ...
                  <input
                    ...
                    autocomplete="off"
                  />
                  <span v-if="errors.name" class="form-err">{{
                    errors.name
                  }}</span>
                </div>
                <div class="form-group">
                  ...
                  <input
                    ...
                    autocomplete="off"
                  />
                  <span v-if="errors.code" class="form-err">{{
                    errors.code
                  }}</span>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Pilih objek yang akan dinilai risikonya.
                </div>
              </div>
            </div>
            <div class="rcc-fields">
              <div
                class="rcc-type-cards"
                :class="{ 'is-error': errors.contextType }"
              >
                <label
                  class="rcc-type-card"
                  :class="{ selected: form.contextType === 'ASSET' }"
                >
                  <input
                    v-model="form.contextType"
                    type="radio"
                    value="ASSET"
                    class="rcc-type-radio"
                  />
                  <div class="rcc-type-icon"><i class="pi pi-server" /></div>
                  <div class="rcc-type-info">
                    <div class="rcc-type-name">Aset</div>
                    <div class="rcc-type-hint">
                      Penilaian risiko berbasis aset organisasi.
                    </div>
                  </div>
                  <div
                    v-if="form.contextType === 'ASSET'"
                    class="rcc-type-check"
                  >
                ...
                <label
                  class="rcc-type-card"
                  :class="{ selected: form.contextType === 'PROCESS' }"
                >
                  <input
                    v-model="form.contextType"
                    type="radio"
                  ...
                  <div class="rcc-type-info">
                    <div class="rcc-type-name">Kegiatan</div>
                    <div class="rcc-type-hint">
                      Penilaian risiko berbasis Kegiatan.
                    </div>
                  </div>
                  <div
                    v-if="form.contextType === 'PROCESS'"
                    class="rcc-type-check"
                  >
                    <i class="pi pi-check-circle" />
                  </div>
              ...
              <span v-if="errors.contextType" class="form-err">{{
                errors.contextType
              }}</span>
            </div>
          </section>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Rentang tahun berlakunya konteks risiko ini.
                </div>
              </div>
            </div>
            <div class="rcc-fields">
              <div class="form-row-2">
                <div class="form-group">
                  <label class="form-label"
                    >Tahun Awal <span class="req">*</span></label
                  >
                  <input
                    v-model.number="form.periodStart"
                    class="rcc-input"
                    :class="{ 'is-error': errors.periodStart }"
                    type="number"
                    min="2000"
                    max="2100"
                  />
                  <span v-if="errors.periodStart" class="form-err">{{
                    errors.periodStart
                  }}</span>
                </div>
                <div class="form-group">
                  <label class="form-label"
                    >Tahun Akhir <span class="req">*</span></label
                  >
                  <input
                    v-model.number="form.periodEnd"
                    class="rcc-input"
                    :class="{ 'is-error': errors.periodEnd }"
                    type="number"
                    min="2000"
                    max="2100"
                  />
                  <span v-if="errors.periodEnd" class="form-err">{{
                    errors.periodEnd
                  }}</span>
                </div>
              </div>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Baris = level kemungkinan, Kolom = level dampak. Nilai 2–10.
                </div>
              </div>
            </div>
            <div class="rcc-fields">
              <div class="rcc-matrix-inputs">
                <div class="form-group">
                  <label class="form-label"
                    >Baris (Kemungkinan) <span class="req">*</span></label
                  >
                  <div class="rcc-stepper">
                    <button
                      type="button"
                      class="rcc-step-btn"
                      :disabled="form.matrixRows <= 2"
                      @click="changeMatrixSize('rows', -1)"
                    >
                      <i class="pi pi-minus" />
                    </button>
                    <input
                      v-model.number="form.matrixRows"
                      class="rcc-input rcc-step-input"
                      :class="{ 'is-error': errors.matrixRows }"
                      type="number"
                      min="2"
                      max="10"
                      @change="onMatrixSizeChange"
                    />
                    <button
                      type="button"
                      class="rcc-step-btn"
                      :disabled="form.matrixRows >= 10"
                      @click="changeMatrixSize('rows', 1)"
                    >
                      <i class="pi pi-plus" />
                    </button>
                  </div>
                  <span v-if="errors.matrixRows" class="form-err">{{
                    errors.matrixRows
                  }}</span>
                </div>
                <div class="form-group">
                  <label class="form-label"
                    >Kolom (Dampak) <span class="req">*</span></label
                  >
                  <div class="rcc-stepper">
                    <button
                      type="button"
                      class="rcc-step-btn"
                      :disabled="form.matrixCols <= 2"
                      @click="changeMatrixSize('cols', -1)"
                    >
                      <i class="pi pi-minus" />
                    </button>
                    <input
                      v-model.number="form.matrixCols"
                      class="rcc-input rcc-step-input"
                      :class="{ 'is-error': errors.matrixCols }"
                      type="number"
                      min="2"
                      max="10"
                      @change="onMatrixSizeChange"
                    ...
                    <button
                      type="button"
                      class="rcc-step-btn"
                      :disabled="form.matrixCols >= 10"
                      @click="changeMatrixSize('cols', 1)"
                    >
                      <i class="pi pi-plus" />
                    </button>
                  </div>
                  <span v-if="errors.matrixCols" class="form-err">{{
                    errors.matrixCols
                  }}</span>
                </div>
                <div class="mx-size-info">
                  <span class="mx-size-badge"
                    >{{ form.matrixRows }}×{{ form.matrixCols }}</span
                  >
                  <span
                    >= {{ form.matrixRows * form.matrixCols }} sel &nbsp;·&nbsp;
                    skor maks
                    <strong>{{
                      form.matrixRows * form.matrixCols
                    }}</strong></span
                  >
                </div>
              </div>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Tentukan kategori tingkat risiko berdasarkan rentang skor
                  (0–{{ form.matrixRows * form.matrixCols }}). Level ini akan
                  mewarnai matriks dan menjadi pilihan selera risiko.
                </div>
              </div>
              <button type="button" class="rl-add-row-btn" @click="addLevelRow">
            ...
            <div class="rcc-fields">
              ...
              <div class="rl-rows">
                <div
                  ...
                  :class="{
                    'rl-row-error':
                      levelRowErrors[idx]?.name || levelRowErrors[idx]?.score,
                  }"
                >
                  <div class="rl-td" style="width: 36px; align-items: center">
                    <div
                      class="rl-order-badge"
                      :style="{
                  ...
                  <div class="rl-td" style="flex: 2">
                    <input
                      ...
                      :class="{ 'is-error': levelRowErrors[idx]?.name }"
                      type="text"
                      placeholder="mis. Rendah"
                    />
                    <span v-if="levelRowErrors[idx]?.name" class="form-err">{{
                      levelRowErrors[idx].name
                    }}</span>
                  </div>
                  <div class="rl-td" style="width: 80px">
                    <input
                      v-model.number="row.minScore"
                      class="rcc-input rl-input"
                      :class="{ 'is-error': levelRowErrors[idx]?.score }"
                      type="number"
                      min="0"
                      :max="form.matrixRows * form.matrixCols"
                    />
                  </div>
                  <div class="rl-td" style="width: 80px">
                    <input
                      v-model.number="row.maxScore"
                      class="rcc-input rl-input"
                      :class="{ 'is-error': levelRowErrors[idx]?.score }"
                      type="number"
                      min="0"
                      :max="form.matrixRows * form.matrixCols"
                    />
                    <span v-if="levelRowErrors[idx]?.score" class="form-err">{{
                      levelRowErrors[idx].score
                    }}</span>
                  </div>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Isi nilai skor untuk setiap kombinasi kemungkinan (baris) dan
                  dampak (kolom). Warna otomatis diterapkan sesuai level risiko
                  di atas.
                </div>
              </div>
              <button
                type="button"
                class="rl-add-row-btn"
                @click="autoFillMatrix"
              >
                          ...
                          :style="{
                            background: matrixGrid[r - 1]?.[c - 1]
                              ? (cellMatchingLevel(
                                ...
                                (matrixGrid[r - 1]![c - 1]!.value ? '33' : '11')
                              : 'transparent',
        ...
        <div class="s2-preview-col">
          <div class="s2-preview-panel">
            ...
            <div class="s2-prev-stats">
              ...
              <div class="s2-prev-stat">
                <div class="s2-prev-stat-val">
                  {{ levelRows.filter((l) => l.name).length }}
                </div>
            ...
            <div
              class="s2-prev-matrix-wrap"
              :style="{
                '--s2-rows': form.matrixRows,
                '--s2-cols': form.matrixCols,
              }"
            >
                    ...
                    >
                      ...
                      >
                        <span
                          ...
                          :style="{
                            color: matrixGrid[r - 1]?.[c - 1]?.value
                              ? cellMatchingLevel(
                                  matrixGrid[r - 1]![c - 1]!.value,
            ...
            <div class="s2-prev-legend">
              <div
                v-if="levelRows.some((l) => l.name)"
                class="s2-prev-legend-items"
              >
                <div
                  v-for="(row, i) in levelRows.filter((l) => l.name)"
                  :key="i"
                  class="s2-prev-legend-item"
                >
                  <span
                    class="s2-prev-legend-dot"
                    :style="{ background: row.color }"
                  />
                  <span class="s2-prev-legend-name">{{ row.name }}</span>
                  <span class="s2-prev-legend-range"
            ...
            <div
              ...
              :class="{
                's2-prev-hint-ok':
                  filledCellCount === form.matrixRows * form.matrixCols,
              }"
            >
              <i
                :class="
                ...
                "
              />
              <span v-if="filledCellCount === form.matrixRows * form.matrixCols"
                >Semua sel terisi — matriks siap</span
              >
              <span v-else
                >{{ form.matrixRows * form.matrixCols - filledCellCount }} sel
                belum terisi. Gunakan "Isi Otomatis" atau isi manual.</span
              >
            </div>
          </div>
        </div>
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                ...
                <div class="rcc-section-desc">
                  Hasil konfigurasi matriks {{ form.matrixRows }}×{{
                    form.matrixCols
                  }}
                    ...
                    <tr v-for="r in form.matrixRows" :key="`mr3-${r}`">
                      ...
                      >
                        <div
                          ...
                          :class="{
                            'mc-cell-appetite': appetiteMatchesCell(r, c),
                          }"
                          :style="{
                            background: matrixGrid[r - 1]?.[c - 1]
                              ? (cellMatchingLevel(
            ...
            <div class="rcc-section-header">
              ...
              <div>
                <div class="rcc-section-title">
                  Selera Risiko <span class="form-opt">(opsional)</span>
                </div>
                <div class="rcc-section-desc">
                  Tentukan level risiko maksimum yang dapat diterima organisasi.
                  Sel matriks yang berada pada atau di bawah level ini akan
                  disorot sebagai zona "aman".
                </div>
              </div>
            </div>
            <div class="rcc-fields">
              <div class="form-group">
                ...
                <div class="rl-select-wrap">
                  <select
                    v-model="riskAppetiteLevel"
                    class="rcc-input rcc-select"
                  >
                    <option value="">— Tidak ditentukan —</option>
                    <option
                      v-for="lvl in levelRows.filter((l) => l.name)"
                      :key="lvl.name"
                      :value="lvl.name"
                    >
                      {{ lvl.name }} (skor {{ lvl.minScore }}–{{
                        lvl.maxScore
                      }})
                    </option>
                  </select>
                  ...
                  <div
                    ...
                    :style="{
                      background: selectedAppetiteLevel.color,
                      boxShadow:
                        '0 0 0 3px ' + selectedAppetiteLevel.color + '44',
                    }"
                  />
                </div>
                <!-- Preview badge -->
                <div
                  v-if="selectedAppetiteLevel"
                  class="rl-appetite-preview"
                  :style="{
                    background: selectedAppetiteLevel.color + '15',
                    borderColor: selectedAppetiteLevel.color + '44',
                  }"
                >
                  <div
                    class="rl-ap-badge"
                    :style="{
                      background: selectedAppetiteLevel.color + '22',
                      color: selectedAppetiteLevel.color,
                      borderColor: selectedAppetiteLevel.color + '55',
                    }"
                  >
                  ...
                  <div
                    class="rl-ap-info"
                    :style="{ color: selectedAppetiteLevel.color }"
                  >
          ...
          <section class="rcc-section">
            <div class="rcc-section-header">
              ...
              <div>
                <div class="rcc-section-title">
                  Opsi Penanganan Risiko
                  <span class="form-opt">(opsional)</span>
                </div>
                <div class="rcc-section-desc">
                  Pilih strategi penanganan yang berlaku pada konteks ini. Opsi
                  yang dipilih akan menjadi referensi dalam proses identifikasi
                  dan evaluasi risiko.
                </div>
              </div>
            </div>
            <div class="rcc-fields">
                ...
                >
                  <div class="to-card-top">
                    ...
                    <div class="to-card-toggle">
                      <div
                        class="to-card-checkbox"
                        :class="{ checked: t.selected }"
                        :style="
                          t.selected
                  ...
                  <div
                    class="to-card-name"
                    :style="t.selected ? { color: t.color } : {}"
                  >
                    {{ t.name }}
                  </div>
              ...
              <div
                v-if="treatmentRows.some((r) => !r.preset)"
                class="to-custom-section"
              >
                ...
                <div class="to-custom-rows">
                  ...
                  >
                    ...
                    <button
                      type="button"
                      class="rl-remove-btn"
                      @click="removeCustomTreatment(t.id)"
                    >
                      <i class="pi pi-trash" />
                    </button>
                  </div>
        ...
        <div class="rcc-action-inner">
          <div class="rcc-action-info">
            <template v-if="step === 1">
              <span v-if="form.name" class="rcc-preview-name">{{
                form.name
              }}</span>
              <span v-if="form.code" class="rcc-preview-code">{{
                form.code
              }}</span>
              <span v-if="!form.name" class="rcc-preview-placeholder"
                >Isi informasi konteks di atas</span
              >
            </template>
            <template v-else-if="step === 2">
              <span class="rcc-preview-code"
                >{{ form.matrixRows }}×{{ form.matrixCols }}</span
              >
              <span class="rcc-preview-name"
                >{{ levelRows.filter((l) => l.name).length }} level risiko</span
              >
              <span class="rcc-preview-placeholder"
                >· {{ filledCellCount }}/{{
                  form.matrixRows * form.matrixCols
                }}
                sel terisi</span
            ...
            <template v-else>
              ...
              <span v-if="riskAppetiteLevel" class="rcc-preview-code"
                >Selera: {{ riskAppetiteLevel }}</span
              >
              <span
                v-if="
                  treatmentRows.filter((t) => t.selected && t.name.trim())
                    .length
                "
                class="rcc-preview-code"
              >
                {{
                  treatmentRows.filter((t) => t.selected && t.name.trim())
                    .length
              ...
              <span v-if="!riskAppetiteLevel" class="rcc-preview-placeholder"
                >Selera risiko belum ditentukan</span
              >
            </template>
          </div>
...
function validateStep1(): boolean {
  ...
  if (!form.contextType) {
    errors.contextType = "Tipe konteks wajib dipilih";
    ok = false;
  }
  if (!form.periodStart || form.periodStart < 2000 || form.periodStart > 2100) {
    errors.periodStart = "Tahun tidak valid (2000–2100)";
    ok = false;
  }
  if (!form.periodEnd || form.periodEnd < 2000 || form.periodEnd > 2100) {
    errors.periodEnd = "Tahun tidak valid (2000–2100)";
    ok = false;
  }
  if (form.periodStart && form.periodEnd && form.periodEnd < form.periodStart) {
    errors.periodEnd = "Tahun akhir tidak boleh sebelum tahun awal";
    ok = false;
  }
  return ok;
}
...
function addLevelRow() {
  ...
  levelRows.value.push({
    ...
    color:
      COLOR_PRESETS[levelRows.value.length % COLOR_PRESETS.length] ?? "#22c55e",
  });
}
...
function changeMatrixSize(axis: "rows" | "cols", delta: number) {
  if (axis === "rows")
    form.matrixRows = Math.min(10, Math.max(2, form.matrixRows + delta));
...
function cellMatchingLevel(value: number): LevelRow | undefined {
  ...
  return levelRows.value.find(
    (l) => l.name && value >= l.minScore && value <= l.maxScore,
  );
}
...
async function submitAll() {
  ...
  try {
    ...
    const selectedTreatments = treatmentRows.value.filter(
      (t) => t.selected && t.name.trim(),
    );
    ...
    const res = await riskContextApi.createFull(frameworkId, {
      ...
      riskAppetiteDescription:
        riskAppetiteDescription.value.trim() || undefined,
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Konteks risiko berhasil dibuat",
      life: 3000,
    });
...
.step-active .rcc-step-num {
  background: rgba(0, 229, 184, 0.15);
  border-color: var(--color-accent);
  color: var(--color-accent);
}
.step-active .rcc-step-label {
  color: var(--color-text);
}
.step-done .rcc-step-num {
...
.rcc-type-cards.is-error .rcc-type-card {
  border-color: var(--color-danger);
}
...
.rcc-step-btn:hover:not(:disabled) {
  background: rgba(0, 229, 184, 0.1);
  color: var(--color-accent);
}
.rcc-step-btn:disabled {
  opacity: 0.4;
  cursor: default;
}
...
.rl-remove-btn:hover:not(:disabled) {
  background: var(--color-danger-dim);
  border-color: rgba(255, 77, 109, 0.3);
  color: var(--color-danger);
}
.rl-remove-btn:disabled {
  opacity: 0.3;
  cursor: default;
}
...
.rcc-btn-cancel:hover:not(:disabled) {
  border-color: var(--color-accent);
  color: var(--color-accent);
}
.rcc-btn-cancel:disabled {
  opacity: 0.5;
  cursor: default;
}
...
.rcc-btn-submit:hover:not(:disabled) {
  background: #00c9a0;
}
.rcc-btn-submit:disabled {
  opacity: 0.6;
  cursor: default;
}
