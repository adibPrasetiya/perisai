    ...
    <template v-else-if="wp">
      ...
      <div v-else class="wpd-rr-filter-bar">
        <div class="wpd-rr-filter-left">
          ...
          >
            <option value="">— Semua {{ filterType === 'asset' ? 'aset' : 'Kegiatan' }} —</option>
                  ...
                  <tr class="wpd-rr-data-row" :class="{ 'wpd-rr-area-subrow': aIdx > 0 }">
                    ...
                    <td class="wpd-rr-td-score wpd-rr-td-score-nilai">
                      <span
                        ...
                        :style="{
                          borderColor: (entry.inherentAssessment?.riskLevel?.color ?? 'var(--color-accent)'),
                          color: (entry.inherentAssessment?.riskLevel?.color ?? 'var(--color-accent)'),
                          background: ((entry.inherentAssessment?.riskLevel?.color ?? '#00e5b8') + '22')
                        }"
                      >
                        {{ areaScore.score }}<template v-if="entry.inherentAssessment?.riskLevel?.name"> • {{ entry.inherentAssessment.riskLevel.name }}</template>
                      </span>
                      <span v-else class="wpd-rr-dash">—</span>
                    </td>
...
.wpd-rr-td-score-nilai { overflow: visible !important; }
