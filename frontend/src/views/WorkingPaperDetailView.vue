  ...
  <div class="wpd-page">
    ...
    <template v-else-if="wp">
      ...
      <div class="wpd-breadcrumb-row">
        <span
          class="wpd-breadcrumb-text"
          @click="router.push({ name: 'working-papers' })"
        >
          Kertas Kerja
        </span>
      ...
      <div class="wpd-header-bar">
        <div class="wpd-header-bar-left">
          ...
          <div class="wpd-header-info">
            ...
            <div class="wpd-header-meta">
              <span
                class="wpd-status-chip"
                :class="`chip-${wp.status.toLowerCase()}`"
              >
              ...
              />
              <span class="wpd-header-meta-text">{{
                wp.program?.name ?? "—"
              }}</span>
              <span v-if="wp.program?.year" class="wpd-year-badge">{{
                wp.program.year
              }}</span>
              <span class="wpd-header-sep">·</span>
              <i
                ...
                style="font-size: 11px; color: var(--color-text-muted)"
              />
              <span v-if="wp.unitKerja?.code" class="wpd-code-badge">{{
                wp.unitKerja.code
              }}</span>
              <span class="wpd-header-meta-text">{{
                wp.unitKerja?.name ?? "—"
        ...
        <div class="wpd-header-bar-right">
          ...
          >
            <i
              :class="
                downloadingReport ? 'pi pi-spin pi-spinner' : 'pi pi-download'
              "
            />
          </button>
          <button
      ...
      <div class="wpd-accordion">
        <button
          class="wpd-accordion-header"
          type="button"
          @click="contextSectionOpen = !contextSectionOpen"
        >
          <i class="pi pi-layer" />
          ...
          <i
            class="pi pi-chevron-down wpd-accordion-chevron"
            :class="{ open: contextSectionOpen }"
          />
        </button>
        <div v-if="contextSectionOpen" class="wpd-accordion-body">
          ...
          <div v-else-if="programFrameworks.length === 0" class="wpd-empty">
            ...
            <p class="wpd-empty-desc">
              Tambahkan framework ke program kerja untuk melihat konteks risiko.
            </p>
          </div>
          <!-- Framework groups -->
          <div v-else class="wpd-fw-groups">
            ...
            >
              <div class="wpd-fw-group-header">
                ...
                <span v-if="pf.framework.version" class="wpd-fw-version"
                  >v{{ pf.framework.version }}</span
                >
                <span class="wpd-fw-group-name">{{ pf.framework.name }}</span>
                <span class="wpd-fw-ctx-count"
                  >{{ (contextsMap[pf.id] ?? []).length }} konteks</span
                >
              </div>
              <div
                v-if="!(contextsMap[pf.id] ?? []).length"
                class="wpd-fw-empty"
              >
              ...
              <div v-else class="wpd-ctx-list">
                <div
                  v-for="ctx in contextsMap[pf.id]"
                  :key="ctx.id"
                  class="wpd-ctx-row"
                >
                  <div class="wpd-ctx-row-left">
                    ...
                    <div class="wpd-ctx-info">
                      ...
                      <span class="wpd-ctx-period"
                        >Periode {{ ctx.periodStart }}–{{ ctx.periodEnd }}</span
                      >
                    </div>
                  </div>
                  <div class="wpd-ctx-row-right">
                    <button
                      type="button"
                      class="btn-icon"
                      title="Lihat detail konteks"
                      @click="goToContext(ctx.id)"
                    >
                      <i class="pi pi-eye" />
                    </button>
                  </div>
      ...
      <div class="wpd-rr-header">
        ...
        <div class="wpd-rr-header-right">
          <button
            ...
            @click="
              router.push({ name: 'risk-entry-create', params: { id: wpId } })
            "
          >
      ...
      <div
        ...
        class="wpd-empty wpd-empty-standalone"
      >
        ...
        <p class="wpd-empty-desc">
          {{
            canEdit
              ? 'Klik "Tambah Entri" untuk memulai identifikasi risiko.'
      ...
      <div v-else class="wpd-rr-filter-bar">
        <div class="wpd-rr-filter-left">
          ...
          >
            <option value="">
              — Semua {{ filterType === "asset" ? "aset" : "Kegiatan" }} —
            </option>
            <option
              v-for="g in availableGroupOptions"
              :key="g.key"
              :value="g.key"
            >
              {{ g.label }}
        ...
        <div class="wpd-rr-filter-right">
          <span class="wpd-rr-filter-count">
            <span class="wpd-rr-filter-count-num">{{
              filteredEntryCount
            }}</span>
        ...
        <table class="wpd-rr-table">
          ...
          <thead>
            <tr class="wpd-rr-thead-top">
              ...
              <th colspan="3" class="wpd-rr-th-group wpd-rr-th-inheren">
                RISIKO INHEREN
              </th>
              <th rowspan="2" class="wpd-rr-th-keputusan">KEPUTUSAN</th>
              <th colspan="3" class="wpd-rr-th-group wpd-rr-th-mitigasi">
                RENCANA MITIGASI
              </th>
              <th rowspan="2" class="wpd-rr-th-status-mitigasi">
                STATUS MITIGASI
              </th>
              <th colspan="3" class="wpd-rr-th-group wpd-rr-th-residu">
                RISIKO RESIDU
              </th>
          ...
          <tbody>
            ...
            <template v-for="group in filteredGroups" :key="group.key">
              <tr class="wpd-rr-group-row">
                <td colspan="18" class="wpd-rr-group-td">
                  <div class="wpd-rr-group-cell">
                    ...
                    <span class="wpd-rr-group-count"
                      >{{ group.entries.length }} risiko</span
                    >
                  </div>
                </td>
              </tr>
              ...
              <template v-for="(entry, idx) in group.entries" :key="entry.id">
                <template
                  v-for="(areaScore, aIdx) in entry.inherentAssessment
                    ?.areaScores?.length
                    ? entry.inherentAssessment.areaScores
                    : [null]"
                  :key="aIdx"
                >
                  <tr
                    class="wpd-rr-data-row"
                    :class="{ 'wpd-rr-area-subrow': aIdx > 0 }"
                  >
                    ...
                    <td
                      ...
                      class="wpd-rr-td-no"
                      :rowspan="
                        entry.inherentAssessment?.areaScores?.length || 1
                      "
                    >
                      {{ idx + 1 }}
                    </td>
                    ...
                    <td
                      ...
                      "
                    >
                      <div v-if="entry.description" class="wpd-rr-cell-text">
                        {{ entry.description }}
                      </div>
                    ...
                    <td
                      v-if="aIdx === 0"
                      class="wpd-rr-td-chip"
                      :rowspan="
                        entry.inherentAssessment?.areaScores?.length || 1
                      "
                    >
                      <span
                        v-if="entry.riskCategory"
                        class="wpd-entry-cat"
                        :style="{
                          borderColor:
                            entry.riskCategory.color ?? 'var(--color-border)',
                          color:
                            entry.riskCategory.color ??
                    ...
                    <td
                      v-if="aIdx === 0"
                      class="wpd-rr-td-kontrol-detail"
                      :rowspan="
                        entry.inherentAssessment?.areaScores?.length || 1
                      "
                    >
                      <template v-if="entry.controls?.length">
                        <div
                          v-for="ctrl in entry.controls"
                          :key="ctrl.id"
                          class="wpd-rr-ctrl-row"
                        >
                          <span class="wpd-rr-ctrl-name">{{ ctrl.name }}</span>
                          <span
                            class="wpd-rr-eff-badge"
                            :class="`eff-${ctrl.effectiveness.toLowerCase()}`"
                          >
                            {{
                    ...
                    <td class="wpd-rr-td-areas">
                      <span v-if="areaScore" class="wpd-rr-area-label">{{
                        areaScore.impactArea.name
                      }}</span>
                      <span v-else class="wpd-rr-dash">—</span>
                    </td>
                    <td class="wpd-rr-td-score">
                      <span v-if="areaScore" class="wpd-rr-score-pill">{{
                        areaScore.likelihoodLevel
                      }}</span>
                      <span v-else class="wpd-rr-dash">—</span>
                    </td>
                    <td class="wpd-rr-td-score">
                      <span v-if="areaScore" class="wpd-rr-score-pill">{{
                        areaScore.impactLevel
                      }}</span>
                      <span v-else class="wpd-rr-dash">—</span>
                    </td>
                    <!-- Nilai inheren: skor + level dengan warna level risiko -->
                    <td class="wpd-rr-td-score wpd-rr-td-score-nilai">
                      <span
                        v-if="areaScore?.score != null"
                        class="wpd-rr-score-pill wpd-rr-score-nilai"
                        :style="{
                          borderColor:
                            entry.inherentAssessment?.riskLevel?.color ??
                            'var(--color-accent)',
                          color:
                            entry.inherentAssessment?.riskLevel?.color ??
                            'var(--color-accent)',
                      ...
                      >
                        {{ areaScore.score
                          ...
                          v-if="entry.inherentAssessment?.riskLevel?.name"
                        >
                          •
                          {{
                            entry.inherentAssessment.riskLevel.name
                          }}</template
                        >
                      </span>
                      <span v-else class="wpd-rr-dash">—</span>
                    ...
                    <td class="wpd-rr-td-keputusan-detail">
                      <template
                        v-if="
                          areaScore &&
                          entry.treatmentPlans?.filter(
                            (p) => p.impactAreaId === areaScore.impactAreaId,
                          ).length
                        "
                      >
                        <div
                          v-for="plan in entry.treatmentPlans.filter(
                            (p) => p.impactAreaId === areaScore.impactAreaId,
                          )"
                          :key="plan.id"
                        ...
                        >
                          ...
                          >
                          <span v-else class="wpd-rr-decision decision-pending"
                            >Belum dipilih</span
                          >
                        </div>
                      </template>
                    ...
                    <td class="wpd-rr-td-text">
                      <template v-if="areaScore">
                        <div
                          v-for="plan in (entry.treatmentPlans ?? []).filter(
                            (p) =>
                              p.impactAreaId === areaScore.impactAreaId &&
                              p.description,
                          )"
                          :key="plan.id"
                    ...
                    <td class="wpd-rr-td-date">
                      <span class="wpd-rr-date">{{
                        formatDate(
                          areaScore
                            ? ((entry.treatmentPlans ?? []).find(
                                (p) =>
                                  p.impactAreaId === areaScore.impactAreaId,
                              )?.targetDate ?? null)
                            : null,
                    ...
                    <td class="wpd-rr-td-pic">
                      ...
                      >
                        {{
                          (entry.treatmentPlans ?? [])
                            .find(
                              (p) => p.impactAreaId === areaScore.impactAreaId,
                            )!
                            .picUserId!.substring(0, 8)
                        }}…
                      </span>
                      <span v-else class="wpd-rr-dash">—</span>
                    </td>
                    ...
                    <td class="wpd-rr-td-status-mitigasi">
                      ...
                      >
                        ...
                        >
                          <span
                            class="wpd-rr-plan-status"
                            :class="`pstatus-${plan.status.toLowerCase()}`"
                          >
                    ...
                    <td class="wpd-rr-td-score">
                      <template
                        v-if="
                          areaScore &&
                          entry.residualAssessment?.areaScores?.find(
                            (r) => r.impactAreaId === areaScore.impactAreaId,
                          )
                        "
                      >
                        <span class="wpd-rr-score-pill">{{
                          entry.residualAssessment!.areaScores.find(
                            (r) => r.impactAreaId === areaScore!.impactAreaId,
                          )!.likelihoodLevel
                        }}</span>
                    ...
                    <td class="wpd-rr-td-score wpd-rr-td-score-nilai">
                      ...
                      >
                        <span
                          ...
                          :style="{
                            borderColor:
                              entry.residualAssessment?.riskLevel?.color ??
                              'var(--color-accent)',
                            color:
                              entry.residualAssessment?.riskLevel?.color ??
                              'var(--color-accent)',
                        ...
                        >
                          {{
                            ...
                            v-if="entry.residualAssessment?.riskLevel?.name"
                          >
                            •
                            {{
                              entry.residualAssessment.riskLevel.name
                            }}</template
                          >
                        </span>
                      </template>
                          ...
                          >
                            ...
                            <template v-if="canEdit">
                              ...
                              <button
                                ...
                                @click="
                                  entry.inherentAssessment &&
                                    openTreatmentModal(entry);
                                  activeActionMenu = null;
                                "
                              >
                                Rencana Penanganan
                              ...
                              <button
                                class="wpd-action-item"
                                :class="{
                                  disabled: !entry.treatmentPlans?.some(
                                    (p) => p.status === 'PLANNED',
                                  ),
                                }"
                                :disabled="
                                  !entry.treatmentPlans?.some(
                                    (p) => p.status === 'PLANNED',
                                  )
                                "
                                @click="
                              ...
                              <div class="wpd-action-divider" />
                              <button
                                class="wpd-action-item wpd-action-item--danger"
                                @click="
      ...
      <div ref="loadMoreSentinel" class="wpd-load-more-sentinel">
        ...
        <span
          ...
          class="wpd-load-more-end"
          >Semua entri telah dimuat</span
        >
      </div>
    </template>
      ...
      <form class="wpd-edit-form" @submit.prevent="submitEdit">
        <div class="wpd-form-group">
          ...
          <input
            ...
            autocomplete="off"
          />
          <span v-if="editTitleError" class="wpd-form-err">{{
            editTitleError
          }}</span>
        </div>
    ...
    >
      <form class="wpd-edit-form" @submit.prevent="submitEditEntry">
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label"
            >Konteks <span class="wpd-req">*</span></label
          >
          <select
            v-model="editEntryForm.programFrameworkContextId"
            class="wpd-form-input"
          >
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label"
            >Kode <span class="wpd-req">*</span></label
          >
          <input
            ...
            autocomplete="off"
          />
          <span v-if="editEntryErrors.code" class="wpd-form-err">{{
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label"
            >Nama Risiko <span class="wpd-req">*</span></label
          >
          <input
            ...
            autocomplete="off"
          />
          <span v-if="editEntryErrors.name" class="wpd-form-err">{{
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label"
            >Deskripsi <span class="wpd-form-opt">(opsional)</span></label
          >
          <textarea
            v-model="editEntryForm.description"
            class="wpd-form-input wpd-form-textarea"
            rows="2"
          />
        </div>
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label">
            ...
            <span v-if="categoryLoading" class="wpd-cat-loading"
              ><i class="pi pi-spin pi-spinner"
            /></span>
          </label>
          <select
            v-model="editEntryForm.riskCategoryId"
            class="wpd-form-input"
            :disabled="categoryLoading"
          >
            <option value="">— Tanpa kategori —</option>
            <option
              v-for="cat in editEntryCategories"
              :key="cat.id"
              :value="cat.id"
            >
              {{ cat.name }}
    ...
    <Dialog
      v-model:visible="showTreatmentPlans"
      modal
      header="Rencana Penanganan"
      :style="{ width: '620px' }"
    >
      <div v-if="treatmentPlansLoading" class="wpd-centered">
        <ProgressSpinner style="width: 28px; height: 28px" />
      </div>
      <div v-else-if="treatmentPlansError" class="wpd-ctx-error">
        {{ treatmentPlansError }}
      </div>
      <template v-else-if="treatmentPlanEntry">
        <p class="wpd-tp-entry-name">{{ treatmentPlanEntry.name }}</p>
        <div
          v-if="!treatmentPlanEntry.treatmentPlans?.length"
          class="wpd-empty"
        ...
        <div
          v-for="plan in treatmentPlanEntry.treatmentPlans"
          :key="plan.id"
          class="wpd-tp-card"
        >
          ...
          <div class="wpd-tp-card-header">
            <span class="wpd-tp-option">{{
              plan.treatmentOption?.name ?? "—"
            }}</span>
            <span
              class="wpd-tp-status-badge"
              :class="`status-${plan.status.toLowerCase().replace(/_/g, '-')}`"
            >
              {{ TREATMENT_STATUS_LABELS[plan.status] }}
            </span>
          </div>
          <p v-if="plan.description" class="wpd-tp-desc">
            {{ plan.description }}
          </p>
          <div class="wpd-tp-meta">
            <span v-if="plan.targetDate"
              >Target: {{ formatDate(plan.targetDate) }}</span
            >
            <span v-if="plan.needsKomiteReview" class="wpd-tp-komite-badge"
              >Perlu Review Komite</span
            >
          </div>
          <div class="wpd-tp-actions">
            <Button
              v-if="
                isOwner && plan.status === 'COMPLETED' && plan.needsKomiteReview
              "
            ...
            <Button
              v-if="
                ...
                (plan.status === 'SUBMITTED_FOR_REVIEW' ||
                  (plan.status === 'COMPLETED' && !plan.needsKomiteReview))
              "
              label="Verifikasi"
              size="small"
              severity="success"
    ...
    >
      <div class="wpd-confirm-body">
        ...
        <p class="wpd-confirm-text">
          Hapus entri risiko
          <strong
            >{{ deleteEntryTarget?.code }} –
            {{ deleteEntryTarget?.name }}</strong
          >?
        </p>
    ...
    <Dialog
      ...
      modal
      header="Detail Entri Risiko"
      :style="{ width: '860px' }"
    >
      <div v-if="expandedEntryLoading" class="wpd-expand-loading">
        <ProgressSpinner style="width: 24px; height: 24px" />
      </div>
      <div v-else-if="expandedEntryError" class="wpd-expand-error">
        {{ expandedEntryError }}
      </div>
      <div v-else-if="expandedEntryDetail" class="wpd-expand-panel">
        ...
        <div class="wpd-ep-section">
          <div class="wpd-ep-section-header">
            ...
            <div
              v-if="expandedEntryDetail.inherentAssessment?.riskLevel"
              class="wpd-ep-risk-summary"
              :style="{
                background:
                  expandedEntryDetail.inherentAssessment.riskLevel.color + '22',
                color:
                  expandedEntryDetail.inherentAssessment.riskLevel.color ??
                  undefined,
              }"
            >
          ...
          <table class="wpd-ep-area-table">
            ...
            <tbody>
              <tr
                v-for="as in expandedEntryDetail.inherentAssessment?.areaScores"
                :key="as.id"
              >
                <td>{{ as.impactArea.name }}</td>
                <td>
                  <span class="wpd-ep-level-pill">{{
                    as.likelihoodLevel
                  }}</span>
                  <span
                    v-if="as.likelihoodDescription"
                    class="wpd-ep-level-desc"
                    >{{ as.likelihoodDescription }}</span
                  >
                </td>
                <td>
                  <span class="wpd-ep-level-pill">{{ as.impactLevel }}</span>
                  <span v-if="as.impactDescription" class="wpd-ep-level-desc">{{
                    as.impactDescription
                  }}</span>
                </td>
                <td>
                  <span class="wpd-ep-score">{{ as.score }}</span>
                </td>
              </tr>
            </tbody>
          </table>
          <p
            v-if="expandedEntryDetail.inherentAssessment?.notes"
            class="wpd-ep-notes"
          >
            <i class="pi pi-info-circle" />
            {{ expandedEntryDetail.inherentAssessment.notes }}
          </p>
        </div>
        ...
        <div class="wpd-ep-section">
          <div class="wpd-ep-section-header">
            ...
            <span class="wpd-ep-count-pill">{{
              expandedEntryDetail.controls?.length ?? 0
            }}</span>
          </div>
          <div
            v-if="!expandedEntryDetail.controls?.length"
            class="wpd-ep-empty"
          >
            Tidak ada kontrol terdaftar.
          </div>
          <div v-else class="wpd-ep-controls-list">
            <div
              v-for="ctrl in expandedEntryDetail.controls"
              :key="ctrl.id"
              class="wpd-ep-control-item"
            >
              <span class="wpd-ep-ctrl-name">{{ ctrl.name }}</span>
              <span
                class="wpd-ep-effectiveness"
                :class="`eff-${ctrl.effectiveness.toLowerCase()}`"
              >
                {{ CONTROL_EFFECTIVENESS_LABELS[ctrl.effectiveness] }}
              </span>
        ...
        <div class="wpd-ep-section">
          <div class="wpd-ep-section-header">
            ...
            <span class="wpd-ep-count-pill">{{
              expandedEntryDetail.treatmentPlans?.length ?? 0
            }}</span>
            <button
              v-if="expandedEntryDetail._count?.treatmentPlans"
              type="button"
              class="wpd-ep-view-plans-btn"
          ...
          <div
            v-if="!expandedEntryDetail.treatmentPlans?.length"
            class="wpd-ep-empty"
          >
            Tidak ada rencana penanganan.
          </div>
          <div v-else class="wpd-ep-plans-summary">
            <div
              v-for="plan in expandedEntryDetail.treatmentPlans"
              :key="plan.id"
              class="wpd-ep-plan-row"
            >
              <span class="wpd-ep-plan-area">{{
                plan.impactArea?.name ?? "—"
              }}</span>
              <span class="wpd-ep-plan-option">{{
                plan.treatmentOption?.name ?? "Opsi belum dipilih"
              }}</span>
              <span
                class="wpd-tp-status-badge"
                :class="`status-${plan.status.toLowerCase().replace(/_/g, '-')}`"
    ...
    <Dialog
      v-model:visible="showInherentModal"
      modal
      header="Penilaian Risiko Inheren"
      :style="{ width: '900px' }"
    >
      <div v-if="inherentContextLoading" class="wpd-centered">
        <ProgressSpinner style="width: 28px; height: 28px" />
      </div>
      <div v-else-if="inherentContextError" class="wpd-ctx-error">
        {{ inherentContextError }}
      </div>
      <template v-else-if="inherentContextDetail">
        ...
        <div class="wpd-inh-tbl-header">
          ...
          <button
            type="button"
            class="wpd-inh-add-btn"
            @click="openAddAreaScore"
          >
        ...
        <div
          v-if="inherentAreaError"
          class="wpd-form-api-error"
        ...
        <div v-if="!inherentScoresList.length" class="wpd-inh-empty">
          <i class="pi pi-chart-bar" />
          <span
            >Belum ada penilaian. Klik "Tambah Penilaian" untuk menilai area
            dampak.</span
          >
        </div>
        ...
        <table v-else class="wpd-inh-tbl">
          ...
          <tbody>
            <tr
              v-for="(item, idx) in inherentScoresList"
              :key="item.impactAreaId"
            >
              <td class="wpd-inh-td-area">
                {{ getAreaNameById(item.impactAreaId) }}
              </td>
              <td class="wpd-inh-td-level">
                <span class="wpd-inh-lvl-num">{{ item.likelihoodLevel }}</span>
                <span class="wpd-inh-lvl-name">{{
                  getLikelihoodLabelById(
                    item.impactAreaId,
                    item.likelihoodLevel,
                  )
                }}</span>
              </td>
              <td class="wpd-inh-td-desc">
                {{
                  getLikelihoodDescById(item.impactAreaId, item.likelihoodLevel)
                }}
              </td>
              <td class="wpd-inh-td-level">
                <span class="wpd-inh-lvl-num">{{ item.impactLevel }}</span>
                <span class="wpd-inh-lvl-name">{{
                  getImpactLabelById(item.impactAreaId, item.impactLevel)
                }}</span>
              </td>
              <td class="wpd-inh-td-desc">
                {{ getImpactDescById(item.impactAreaId, item.impactLevel) }}
              </td>
              <td class="wpd-inh-td-score">
                <span
                  class="wpd-inh-score-chip"
              ...
              <td class="wpd-inh-td-aksi">
                <button
                  class="wpd-inh-row-btn"
                  title="Edit"
                  @click="openEditAreaScore(idx)"
                >
                  <i class="pi pi-pencil" />
                </button>
                <button
                  class="wpd-inh-row-btn wpd-inh-row-btn--danger"
                  title="Hapus"
                  @click="removeAreaScore(idx)"
                >
                  <i class="pi pi-trash" />
                </button>
              </td>
        ...
        <div v-if="inherentOverallScore !== null" class="wpd-inh-score-summary">
          ...
          <span
            class="wpd-inh-score-value"
            :style="{
              color: inherentOverallRiskLevel?.color ?? 'var(--color-accent)',
            }"
          >
            {{ inherentOverallScore }}
          </span>
          <span
            v-if="inherentOverallRiskLevel"
            class="wpd-inh-level-pill"
            :style="{
              background: (inherentOverallRiskLevel.color ?? '#64748b') + '22',
              color: inherentOverallRiskLevel.color ?? '#64748b',
              borderColor: (inherentOverallRiskLevel.color ?? '#64748b') + '44',
            }"
          >
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          :disabled="inherentSubmitLoading"
          @click="showInherentModal = false"
        />
        <Button
          label="Simpan Penilaian"
          :loading="inherentSubmitLoading"
          :disabled="inherentContextLoading || !!inherentContextError"
          @click="submitInherentAssessment"
        />
      </template>
    </Dialog>
    ...
    <Dialog
      ...
      modal
      :header="
        editingAreaScoreIndex !== null ? 'Edit Penilaian' : 'Tambah Penilaian'
      "
      :style="{ width: '500px' }"
    >
      <!-- Area Dampak -->
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >Area Dampak <span class="wpd-req">*</span></label
        >
        <select
          ...
          @change="
            areaScoreSubForm.likelihoodLevel = 0;
            areaScoreSubForm.impactLevel = 0;
          "
        >
        ...
        <span v-if="areaScoreSubErrors.impactAreaId" class="wpd-form-err">{{
          areaScoreSubErrors.impactAreaId
        }}</span>
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >Kemungkinan <span class="wpd-req">*</span></label
        >
        <div v-if="!areaScoreSubForm.impactAreaId" class="wpd-crit-placeholder">
          Pilih area dampak terlebih dahulu
        </div>
        ...
        <span v-if="areaScoreSubErrors.likelihoodLevel" class="wpd-form-err">{{
          areaScoreSubErrors.likelihoodLevel
        }}</span>
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >Dampak <span class="wpd-req">*</span></label
        >
        <div v-if="!areaScoreSubForm.impactAreaId" class="wpd-crit-placeholder">
          Pilih area dampak terlebih dahulu
        </div>
        <div v-else class="wpd-crit-list">
          <button
            ...
            class="wpd-crit-item"
            :class="{
              'is-selected': areaScoreSubForm.impactLevel === crit.level,
            }"
        ...
        <span v-if="areaScoreSubErrors.impactLevel" class="wpd-form-err">{{
          areaScoreSubErrors.impactLevel
        }}</span>
      </div>
      ...
      <div v-if="subModalPreviewScore !== null" class="wpd-inh-preview-score">
        ...
        <span
          class="wpd-inh-score-chip"
          :style="getScoreBadgeStyle(subModalPreviewScore)"
          >{{ subModalPreviewScore }}</span
        >
      </div>
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          @click="showAreaScoreSubModal = false"
        />
        <Button
          :label="editingAreaScoreIndex !== null ? 'Simpan' : 'Tambahkan'"
          @click="confirmAreaScore"
        />
      </template>
    </Dialog>
    ...
    <Dialog
      v-model:visible="showResidualModal"
      modal
      header="Tandai Mitigasi Selesai"
      :style="{ width: '700px' }"
    >
      <div v-if="residualContextLoading" class="wpd-centered">
        <ProgressSpinner style="width: 28px; height: 28px" />
      </div>
      <div v-else-if="residualContextError" class="wpd-ctx-error">
      ...
      <template v-else-if="residualContextDetail">
        ...
        <div class="wpd-form-group">
          <label class="wpd-form-label"
            >Pilih Rencana Penanganan yang Selesai
            <span class="wpd-req">*</span></label
          >
          <div
            v-if="!plannedPlansWithInherentScore.length"
            class="wpd-crit-placeholder"
          >
            Tidak ada rencana penanganan PLANNED yang memiliki penilaian
            inheren.
          </div>
          <select
            v-else
            v-model="residualSelectedPlanId"
            class="wpd-form-input"
            @change="onResidualPlanChange"
          >
            <option value="">— Pilih rencana penanganan —</option>
            <option
              v-for="plan in plannedPlansWithInherentScore"
              :key="plan.id"
              :value="plan.id"
            >
              {{ plan.impactArea?.name ?? "—"
                ...
                plan.description ? ` – ${plan.description.slice(0, 60)}` : ""
              }}
            </option>
          </select>
          <span v-if="residualFormErrors.planId" class="wpd-form-err">{{
            residualFormErrors.planId
          }}</span>
        </div>
        ...
        <template v-if="residualSelectedPlan">
          ...
          <div class="wpd-res-inherent-ref">
            <span class="wpd-res-inherent-ref-label"
              >Skor Inheren Area Ini:</span
            >
            <span
              class="wpd-inh-score-chip"
              :style="getResidualScoreBadgeStyle(residualInherentScore!)"
            >
              {{ residualInherentScore }}
            </span>
          </div>

          <div class="wpd-inh-tbl-label" style="margin: 0.75rem 0 0.5rem">
            Penilaian Risiko Residual — {{ residualSelectedArea?.name }}
          </div>
          ...
          <div class="wpd-form-group">
            ...
            <div class="wpd-crit-list">
              <button
                ...
                class="wpd-crit-item"
                :class="{
                  'is-selected': residualForm.likelihoodLevel === crit.level,
                }"
            ...
            <span
              v-if="residualFormErrors.likelihoodLevel"
              class="wpd-form-err"
              >{{ residualFormErrors.likelihoodLevel }}</span
            >
          </div>
          ...
          <div class="wpd-form-group">
            ...
            <div class="wpd-crit-list">
              <button
                ...
                class="wpd-crit-item"
                :class="{
                  'is-selected': residualForm.impactLevel === crit.level,
                }"
            ...
            <span v-if="residualFormErrors.impactLevel" class="wpd-form-err">{{
              residualFormErrors.impactLevel
            }}</span>
          </div>

          <!-- Preview skor -->
          <div
            v-if="residualPreviewScore !== null"
            class="wpd-inh-preview-score"
          >
            <span class="wpd-inh-preview-label">Skor Residual:</span>
            <span
              class="wpd-inh-score-chip"
              :style="getResidualScoreBadgeStyle(residualPreviewScore)"
              >{{ residualPreviewScore }}</span
            >
          </div>
        </template>

        <div
          v-if="residualSubmitError"
          class="wpd-form-api-error"
          style="margin-top: 0.75rem"
        >
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          :disabled="residualSubmitLoading"
          @click="showResidualModal = false"
        />
        <Button
          label="Tandai Selesai & Simpan Penilaian"
          :loading="residualSubmitLoading"
          :disabled="
            residualContextLoading ||
            !!residualContextError ||
            !residualSelectedPlanId
          "
    ...
    <Dialog
      ...
      :style="{ width: '900px' }"
    >
      <div v-if="treatmentModalLoading" class="wpd-centered">
        <ProgressSpinner style="width: 28px; height: 28px" />
      </div>
      <div v-else-if="treatmentModalError" class="wpd-ctx-error">
        {{ treatmentModalError }}
      </div>
      <template v-else>
        ...
        <div v-if="!treatmentPlansForm.length" class="wpd-inh-empty">
          ...
          <span
            >Belum ada rencana penanganan. Klik "Tambah Rencana" untuk
            menambahkan.</span
          >
        </div>
        ...
        <table v-else class="wpd-inh-tbl">
          ...
          <tbody>
            <tr v-for="(plan, idx) in treatmentPlansForm" :key="idx">
              <td class="wpd-inh-td-area">
                {{ getTreatmentAreaName(plan.impactAreaId) }}
              </td>
              <td class="wpd-tp-td-option">
                {{ getTreatmentOptionName(plan.treatmentOptionId) }}
              </td>
              <td class="wpd-inh-td-desc">{{ plan.description || "—" }}</td>
              <td class="wpd-tp-td-pic">
                {{ getTreatmentPicName(plan.picUserId) }}
              </td>
              ...
              <td class="wpd-inh-td-aksi">
                <button
                  class="wpd-inh-row-btn"
                  title="Edit"
                  @click="openEditTreatmentPlan(idx)"
                >
                  <i class="pi pi-pencil" />
                </button>
                <button
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          :disabled="treatmentSubmitLoading"
          @click="showTreatmentModal = false"
        />
        <Button
          label="Simpan"
          :loading="treatmentSubmitLoading"
          :disabled="treatmentModalLoading || !!treatmentModalError"
          @click="submitTreatmentPlans"
        />
      </template>
    </Dialog>
    ...
    >
      ...
      <div class="wpd-form-group">
        ...
        <span v-if="treatmentSubErrors.impactAreaId" class="wpd-form-err">{{
          treatmentSubErrors.impactAreaId
        }}</span>
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label">
          ...
          >
            ({{
              subModalAreaScore <= appetiteThreshold.maxScore
                ? "wajib Diterima"
        ...
        >
          ...
          <span v-if="subModalAreaScore <= appetiteThreshold.maxScore">
            Skor area ini ({{ subModalAreaScore }}) berada dalam selera risiko —
            hanya opsi <strong>Diterima</strong> yang tersedia.
          </span>
          <span v-else>
            Skor area ini ({{ subModalAreaScore }}) melebihi selera risiko —
            opsi <strong>Diterima</strong> tidak tersedia.
          </span>
        </div>
        <select
          v-model="treatmentSubForm.treatmentOptionId"
          class="wpd-form-input"
        >
          <option value="">— Tanpa opsi spesifik —</option>
          <option
            v-for="opt in availableTreatmentOptionsForSubModal"
            :key="opt.id"
            :value="opt.id"
          >
        ...
        <p
          v-if="treatmentSubSelectedOption?.isAcceptance"
          class="wpd-tp-acceptance-note"
        >
          <i class="pi pi-info-circle" />
          <span>
            Opsi ini menandakan risiko <strong>Diterima</strong> — status
            mitigasi akan otomatis <strong>Selesai</strong> dan penilaian
            residual akan disalin dari inheren saat disimpan.
          </span>
        </p>
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >Deskripsi Rencana <span class="wpd-form-opt">(opsional)</span></label
        >
        <textarea
          v-model="treatmentSubForm.description"
          class="wpd-form-input wpd-form-textarea"
          rows="3"
        />
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >PIC Pelaksana <span class="wpd-form-opt">(opsional)</span></label
        >
        <input
          v-model="treatmentSubForm.picUserId"
          type="text"
          class="wpd-form-input"
          placeholder="Nama PIC pelaksana"
        />
      </div>
      ...
      <div class="wpd-form-group">
        <label class="wpd-form-label"
          >Target Selesai <span class="wpd-form-opt">(opsional)</span></label
        >
        <input
          v-model="treatmentSubForm.targetDate"
          type="date"
          class="wpd-form-input"
        />
      </div>
      ...
      <template #footer>
        <Button
          label="Batal"
          severity="secondary"
          @click="showTreatmentSubModal = false"
        />
        <Button
          :label="editingTreatmentPlanIndex !== null ? 'Simpan' : 'Tambahkan'"
          @click="confirmTreatmentPlan"
        />
      </template>
    </Dialog>
  </div>
...
import {
  workingPaperApi,
  WORKING_PAPER_STATUS_LABELS,
  type WorkingPaper,
  type WpStats,
} from "@/api/workingPaper";
...
import {
  programFrameworkContextApi,
  type ProgramFrameworkContext,
} from "@/api/programFrameworkContext";
import {
  riskEntryApi,
...
const isReviewer = computed(
  () =>
    auth.user?.roles?.includes("KOMITE_PUSAT") ||
...
const groupedEntries = computed((): EntryGroup[] => {
  ...
  for (const entry of entries.value) {
    ...
    if (entry.asset) {
      ...
      label = entry.businessProcess.name;
      type = "process";
    } else {
  ...
  return [...map.values()]
    .map((g) => ({
      ...g,
      entries: [...g.entries].sort((a, b) => a.order - b.order),
    }))
...
const filteredGroups = computed(() => {
  ...
  if (filterType.value !== "all")
    groups = groups.filter((g) => g.type === filterType.value);
  if (filterGroupKey.value)
    groups = groups.filter((g) => g.key === filterGroupKey.value);
  return groups;
});
...
async function openDetailModal(entry: RiskEntry) {
  ...
  try {
    ...
    expandedEntryError.value = extractApiError(
      err,
...
async function loadContexts(programId: string) {
  ...
  try {
    ...
    ctxResults.forEach((r) => {
      ctxMap[r.pfId] = r.contexts;
    });
    contextsMap.value = ctxMap;

    const pMap: Record<string, ProgramFrameworkContext[]> = {};
    pfcResults.forEach((r) => {
      pMap[r.pfId] = r.pfcs;
    });
    pfcMap.value = pMap;
  } catch (err: any) {
    contextsError.value = extractApiError(
      err,
      "Gagal memuat konteks risiko program.",
    );
  } finally {
...
function goToContext(contextId: string) {
  router.push({
    ...
    params: { contextId },
    query: { from: "working-paper-detail", wpId },
  });
}
...
async function downloadReport() {
  ...
  try {
    ...
    toast.add({
      severity: "error",
      summary: "Gagal",
      detail: extractApiError(err, "Gagal mengunduh laporan PDF."),
      life: 4000,
    });
  } finally {
...
async function submitEdit() {
  ...
  editError.value = "";
  try {
    const res = await workingPaperApi.update(wpId, {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Kertas kerja berhasil diperbarui",
      life: 3000,
    });
  } catch (err: any) {
...
async function loadEntries() {
  ...
  entriesError.value = "";
  try {
    const res = await riskEntryApi.listByWorkingPaper(wpId, {
      page: 1,
      limit: PAGE_LIMIT,
    });
    ...
    entriesError.value = extractApiError(
      err,
      "Gagal memuat daftar entri risiko.",
    );
  } finally {
...
const editEntryCategories = computed(() => {
  const pfc = pfcOptions.value.find(
    (o) => o.id === editEntryForm.programFrameworkContextId,
  );
  if (!pfc) return [];
...
watch(
  () => editEntryForm.programFrameworkContextId,
  (pfcId) => {
    if (!pfcId) return;
...
function openEditEntry(entry: RiskEntry) {
  ...
  const pfc = pfcOptions.value.find(
    (o) => o.id === entry.programFrameworkContextId,
  );
  if (pfc) loadCategoriesForContext(pfc.riskContextId);
...
async function submitEditEntry() {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Entri risiko berhasil diperbarui",
      life: 3000,
    });
    loadEntries();
    loadStats();
  } catch (err: any) {
    editEntryError.value = extractApiError(
      err,
      "Gagal memperbarui entri risiko.",
    );
  } finally {
...
async function doDeleteEntry() {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Entri risiko berhasil dihapus",
      life: 3000,
    });
    loadEntries();
    loadStats();
    ...
    deleteEntryError.value = extractApiError(
      err,
      "Gagal menghapus entri risiko.",
    );
  } finally {
...
async function openTreatmentPlans(entry: RiskEntry) {
  ...
  try {
    ...
    treatmentPlansError.value = extractApiError(
      err,
      "Gagal memuat rencana penanganan.",
    );
  } finally {
...
async function doSubmitPlan(planId: string) {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Rencana diajukan ke Komite Pusat",
      life: 3000,
    });
    ...
    toast.add({
      severity: "error",
      summary: "Error",
      detail: extractApiError(err, "Gagal mengajukan rencana."),
      life: 4000,
    });
  } finally {
...
async function doVerifyPlan(planId: string) {
  ...
  try {
    ...
    toast.add({
      ...
      detail: "Rencana terverifikasi dan kontrol telah dibuat",
      life: 3000,
    });
    const res = await riskEntryApi.getById(treatmentPlanEntry.value.id);
    treatmentPlanEntry.value = res.data.data;
    ...
    toast.add({
      severity: "error",
      summary: "Error",
      detail: extractApiError(err, "Gagal memverifikasi rencana."),
      life: 4000,
    });
  } finally {
...
const areaScoreSubForm = ref({
  impactAreaId: "",
  likelihoodLevel: 0,
  impactLevel: 0,
});
const areaScoreSubErrors = ref<{
  impactAreaId?: string;
...
const availableAreasForSubModal = computed<ImpactArea[]>(() => {
  ...
  if (editingAreaScoreIndex.value !== null) {
    ...
    usedIds.delete(
      inherentScoresList.value[editingAreaScoreIndex.value]?.impactAreaId ?? "",
    );
  }
  return inherentContextDetail.value.impactAreas.filter(
    (a) => !usedIds.has(a.id),
  );
});

const subModalSelectedArea = computed<ImpactArea | null>(() => {
  if (!areaScoreSubForm.value.impactAreaId || !inherentContextDetail.value)
    return null;
  return (
    inherentContextDetail.value.impactAreas.find(
      (a) => a.id === areaScoreSubForm.value.impactAreaId,
    ) ?? null
  );
});
...
function getAreaNameById(areaId: string): string {
  return (
    inherentContextDetail.value?.impactAreas.find((a) => a.id === areaId)
      ?.name ?? areaId
  );
}

function getLikelihoodLabelById(areaId: string, level: number): string {
  const area = inherentContextDetail.value?.impactAreas.find(
    (a) => a.id === areaId,
  );
  return (
    area?.likelihoodCriteria?.find((c) => c.level === level)?.name ??
    `Level ${level}`
  );
}
...
function getLikelihoodDescById(areaId: string, level: number): string {
  ...
  return (
    area?.likelihoodCriteria?.find((c) => c.level === level)?.description ?? "—"
  );
}
...
function getImpactLabelById(areaId: string, level: number): string {
  ...
  return (
    area?.impactCriteria?.find((c) => c.level === level)?.name ??
    `Level ${level}`
  );
}
...
function getImpactDescById(areaId: string, level: number): string {
  ...
  return (
    area?.impactCriteria?.find((c) => c.level === level)?.description ?? "—"
  );
}

function getScoreBadgeStyle(score: number) {
  const rl = inherentContextDetail.value?.riskLevels?.find(
...
function openEditAreaScore(idx: number) {
  ...
  if (!item) return;
  areaScoreSubForm.value = {
    impactAreaId: item.impactAreaId,
...
function confirmAreaScore() {
  ...
  if (!impactAreaId) {
    areaScoreSubErrors.value.impactAreaId = "Pilih area dampak";
    return;
  }
  if (!likelihoodLevel) {
    areaScoreSubErrors.value.likelihoodLevel = "Pilih level kemungkinan";
    return;
  }
  if (!impactLevel) {
    areaScoreSubErrors.value.impactLevel = "Pilih level dampak";
    return;
  }
  const score = calcScore(impactAreaId, likelihoodLevel, impactLevel);
  if (editingAreaScoreIndex.value !== null) {
    inherentScoresList.value[editingAreaScoreIndex.value] = {
      impactAreaId,
      likelihoodLevel,
      impactLevel,
      score,
...
const inherentOverallRiskLevel = computed(() => {
  if (inherentOverallScore.value === null || !inherentContextDetail.value)
    return null;
  return (
    inherentContextDetail.value.riskLevels?.find(
      (rl) =>
        inherentOverallScore.value! >= rl.minScore &&
        inherentOverallScore.value! <= rl.maxScore,
    ) ?? null
  );
});
...
async function openInherentModal(entry: RiskEntry) {
  ...
  try {
    ...
    if (entry.inherentAssessment?.areaScores?.length) {
      inherentScoresList.value = entry.inherentAssessment.areaScores.map(
        (as) => ({
    ...
    inherentContextError.value = extractApiError(
      err,
      "Gagal memuat konteks risiko.",
    );
  } finally {
...
async function submitInherentAssessment() {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Penilaian risiko inheren berhasil disimpan",
      life: 3000,
    });
    ...
    inherentSubmitError.value = extractApiError(
      err,
...
const plannedPlansWithInherentScore = computed(() => {
  const inherentAreaIds = new Set(
    residualModalEntry.value?.inherentAssessment?.areaScores?.map(
      (s) => s.impactAreaId,
    ) ?? [],
  );
  return (residualModalEntry.value?.treatmentPlans ?? []).filter(
    (p) =>
      p.status === "PLANNED" &&
      p.impactAreaId &&
      inherentAreaIds.has(p.impactAreaId),
  );
});

const residualSelectedPlan = computed(
  () =>
    plannedPlansWithInherentScore.value.find(
      (p) => p.id === residualSelectedPlanId.value,
    ) ?? null,
);
...
const residualSelectedArea = computed<ImpactArea | null>(() => {
  ...
  return (
    residualContextDetail.value.impactAreas.find(
      (a) => a.id === plan.impactAreaId,
    ) ?? null
  );
});
...
const residualInherentScore = computed<number | null>(() => {
  ...
  return (
    residualModalEntry.value?.inherentAssessment?.areaScores?.find(
...
const residualPreviewScore = computed<number | null>(() => {
  ...
  if (
    !residualSelectedPlan.value?.impactAreaId ||
    !likelihoodLevel ||
    !impactLevel
  )
    return null;
  return calcResidualScore(
    residualSelectedPlan.value.impactAreaId,
    likelihoodLevel,
    impactLevel,
  );
...
function getResidualScoreBadgeStyle(score: number) {
  const rl = residualContextDetail.value?.riskLevels?.find(
    (r) => score >= r.minScore && score <= r.maxScore,
  );
  const color = rl?.color ?? "#64748b";
  return { background: color + "22", color, borderColor: color + "55" };
...
async function openResidualModal(entry: RiskEntry) {
  ...
  try {
    ...
    residualContextError.value = extractApiError(
      err,
      "Gagal memuat konteks risiko.",
    );
  } finally {
...
async function submitResidualAssessment() {
  ...
  try {
    ...
    toast.add({
      ...
      summary: "Berhasil",
      detail:
        "Mitigasi ditandai selesai dan penilaian residual berhasil disimpan",
      life: 3000,
    });
    ...
    residualSubmitError.value = extractApiError(
      err,
      "Gagal menyimpan penilaian.",
    );
  } finally {
...
const availableAreasForTreatmentSubModal = computed<ScoredArea[]>(() => {
  ...
  if (editingTreatmentPlanIndex.value !== null) {
    usedIds.delete(
      treatmentPlansForm.value[editingTreatmentPlanIndex.value]?.impactAreaId ??
        "",
    );
  }
...
const treatmentSubForm = ref<TreatmentPlanForm>({
  impactAreaId: "",
  treatmentOptionId: "",
  description: "",
  picUserId: "",
...
const subModalAreaScore = computed<number | null>(() => {
  ...
  return (
    (treatmentModalEntry.value?.inherentAssessment?.areaScores ?? []).find(
      (s) => s.impactAreaId === areaId,
    )?.score ?? null
  );
});
...
watch(
  () => treatmentSubForm.value.impactAreaId,
  (newAreaId) => {
    if (!newAreaId || !appetiteThreshold.value) return;
    const score = (
      treatmentModalEntry.value?.inherentAssessment?.areaScores ?? []
    ).find((s) => s.impactAreaId === newAreaId)?.score;
    if (score == null) return;
    const currentOpt =
      treatmentModalContextDetail.value?.treatmentOptions?.find(
        (o) => o.id === treatmentSubForm.value.treatmentOptionId,
      );
    // Clear option if it violates the appetite rule
    if (currentOpt) {
      const invalid =
        (score <= appetiteThreshold.value.maxScore &&
          !currentOpt.isAcceptance) ||
    ...
    if (
      score <= appetiteThreshold.value.maxScore &&
      !treatmentSubForm.value.treatmentOptionId
    ) {
      const acceptanceOpts = (
        treatmentModalContextDetail.value?.treatmentOptions ?? []
      ).filter((o) => o.isAcceptance);
      if (acceptanceOpts.length === 1)
        treatmentSubForm.value.treatmentOptionId = acceptanceOpts[0]!.id;
    }
  },
);
...
const treatmentSubSelectedOption = computed(() => {
  if (
    !treatmentSubForm.value.treatmentOptionId ||
    !treatmentModalContextDetail.value
  )
    return null;
  return (
    treatmentModalContextDetail.value.treatmentOptions?.find(
...
function getTreatmentAreaName(areaId: string): string {
  return (
    treatmentScoredAreas.value.find((a) => a.impactAreaId === areaId)?.name ??
    areaId
  );
}
...
function openEditTreatmentPlan(idx: number) {
  ...
  if (
    ...
    const areaScore = (
      ...
      (s) => s.impactAreaId === treatmentSubForm.value.impactAreaId,
    )?.score;
...
function confirmTreatmentPlan() {
  ...
  if (editingTreatmentPlanIndex.value !== null) {
    treatmentPlansForm.value[editingTreatmentPlanIndex.value] = {
      ...treatmentSubForm.value,
    };
  } else {
...
async function openTreatmentModal(entry: RiskEntry) {
  ...
  try {
    ...
    const existingPlans = (entry.treatmentPlans ?? []).filter(
      (p) => p.status === "PLANNED",
    );
...
async function submitTreatmentPlans() {
  ...
  try {
    ...
    toast.add({
      severity: "success",
      summary: "Berhasil",
      detail: "Rencana penanganan berhasil disimpan",
      life: 3000,
    });
    ...
    treatmentSubmitError.value = extractApiError(
      err,
      "Gagal menyimpan rencana penanganan.",
    );
  } finally {
...
.wpd-accordion-header:hover {
  background: var(--color-bg-alt);
}
.wpd-accordion-header .pi:first-child {
  color: var(--color-accent);
  font-size: 13px;
}
...
.wpd-rr-td-keputusan-detail {
  vertical-align: top !important;
  padding: 6px 8px !important;
}
.wpd-rr-td-status-mitigasi {
  vertical-align: top !important;
  padding: 6px 8px !important;
}
.wpd-rr-plan-row {
...
.wpd-ep-area-table td {
  padding: 6px 8px;
  border-bottom: 1px solid var(--color-border);
}
.wpd-ep-area-table tr:last-child td {
  border-bottom: none;
}
.wpd-ep-level-pill {
  display: inline-flex;
...
.wpd-inh-row-btn:hover {
  border-color: var(--color-accent);
  color: var(--color-accent);
}
.wpd-inh-row-btn--danger:hover {
  border-color: #f87171;
  color: #f87171;
}
