...
const registration = async (reqBody) => {
  ...
  await activityLog.notice("USER_CREATED", {
    actionType: "CREATE",
...
const login = async (reqBody, userAgent, ipAddress) => {
  ...
  if (!user) {
    await activityLog.warning("LOGIN_FAILURE", {
      actionType: "AUTH",
      ...loginContext,
  ...
  if (!user.totpEnabled) {
    ...
    await activityLog.info("TOTP_SETUP_REQUIRED", {
      actionType: "AUTH",
      userId: user.id,
      username: user.username,
      ipAddress,
    ...
    await activityLog.info("TOTP_VERIFY_REQUIRED", {
      actionType: "AUTH",
      userId: user.id,
      username: user.username,
      ipAddress,
...
const verifyLoginTotp = async (reqBody, userAgent, ipAddress) => {
  ...
  await activityLog.notice("LOGIN_SUCCESS", {
    actionType: "AUTH",
    userId: user.id,
    username: user.username,
    ipAddress,
...
const resetPasswordWithTotp = async (reqBody) => {
  ...
  await activityLog.notice("PASSWORD_RESET", {
    actionType: "AUTH",
    userId: user.id,
    username: user.username,
  });
...
const verifyTotpSetup = async (reqBody, userAgent, ipAddress) => {
  ...
  await activityLog.notice("TOTP_SETUP_COMPLETE", {
    actionType: "AUTH",
    userId: user.id,
    username: user.username,
    ipAddress,
...
const logout = async (username) => {
  ...
  await activityLog.info("LOGOUT", { actionType: "AUTH", username });
